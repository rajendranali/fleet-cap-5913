import React ,{useContext}from 'react'
import {Box,Button,Text,Flex,Stack,Image} from "@chakra-ui/react"
import {NavLink,Link,useNavigate} from "react-router-dom"
import styles  from "./Nav.module.css"
import { AuthContext } from '../AuthContext/AuthContext'
import User from "./User"
const Navbar = () => {
  let navigate=useNavigate()
  const {state,dispatch} = useContext(AuthContext)
  //console.log(state)
  let styleList={
    color:"rgb(49,103,255)"

    
  }
  
return (
    <Box className={styles.navbar}>
        <Stack  direction={['column', 'row']} justifyContent="space-around" alignItems="center">
            <Box>
            <Link to="/">
                <Image  w ="150px" h="100px"src='data:image/png;base64,'/>
                </Link>
            </Box>
            <Stack gap="40px" direction={['column', 'row']}>
              <Link to="/"></Link>
            <NavLink className={styles.container} to="/menu"style={({ isActive }) =>
              isActive ? styleList : undefined
            }>Plan & Menu</NavLink>
           <NavLink to="/work"className={styles.container}style={({ isActive }) =>
              isActive ? styleList : undefined
            }>How it Works</NavLink>
           <NavLink to="/gift"className={styles.container}style={({ isActive }) =>
              isActive ? styleList : undefined
            }>Gift</NavLink>
           <NavLink to="/refer"className={styles.container}style={({ isActive }) =>
              isActive ? styleList : undefined
            }>Refer a Friend</NavLink>
           <NavLink to="/help"className={styles.container}style={({ isActive }) =>
              isActive ? styleList : undefined
            }>Help</NavLink>
            </Stack>
            <Stack direction={['column', 'row']}>
                <Button bg="white" className={styles.loginbtn} onClick={()=>navigate("login")}>{state.isAuth?<User/>:"Login"}</Button>

                <Button bg="rgb(49,103,255)" color="White" borderRadius="0px" className={styles.signbtn}onClick={()=>navigate("signup")}>Signup</Button>
            </Stack>


        </Stack>
    </Box>
    
  )
}

export default Navbar